...
class DataLoader(object):
    ...
    def __next__(self):        
        self._next_minibatch_indices = self._minibatch_indices.copy()
        if self.sequential:
    ...
    def get_next_state(self):
        indexes = self._next_minibatch_indices + 1

        try:
            if np.array(np.where(indexes >= len(self.observations))).any():
                idxs = np.where(indexes >= len(self.observations))
                indexes = np.delete(indexes, idxs)
                return self.observations[indexes], idxs
        except IndexError as exception:
            print(exception)
            print(indexes)
            exit()  

        return self.observations[indexes], None
